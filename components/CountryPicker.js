...
export default function CountryPicker({
  ...
  const resultCountries = React.useMemo(() => {
    ...
    return countryCodes.filter(country =>
      country?.name[lang || 'en'].match(new RegExp(searchValue, 'i')),
    );
  }, [searchValue]);
      ...
      <View
        ...
        <View style={styles.modalApply}>
          {applyText ? (
            <TouchableOpacity
              ...
              <Text style={applyTextStyle ? applyTextStyle : {fontSize: 16}}>
                {applyText ? applyText : 'Apply'}
              </Text>
            </TouchableOpacity>
          ) : null}
        </View>
        <View style={styles.modalCancel}>
          <TouchableOpacity
            ...
            <Text style={cancelTextStyle ? cancelTextStyle : {fontSize: 16}}>
